...
while True:
  ...
  'authorization': f"Bearer {token}"
        ...
        if "errorMessage" in response_json:
            print(f"{Fore.YELLOW}CODE: {coupon_number} - MESSAGE: {response_json['errorMessage']}{Style.RESET_ALL}")
            print(ff.text)
        else:
            print(f"{Fore.GREEN}CODE: {coupon_number} - MESSAGE: {response_json}{Style.RESET_ALL}")
            break 
